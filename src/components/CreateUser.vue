    ...
    <div class="card">
        <div class="card-content">
            ...
            <div class="field">
                ...
                <div class="control">
                    <input v-validate="'required|alpha'" v-model="EmployeeId" type="text" name="EmployeeId" id="EmployeeId" class="input" :class="{'input': true, 'is-danger': errors.has('EmployeeId') }" />
            ...
            <div class="field">
                ...
                <div class="control">
                    <input v-validate="'required|alpha'" v-model="Department" type="text" name="Department" id="Department" class="input" :class="{'input': true, 'is-danger': errors.has('Department') }" />
                    <span v-show="errors.has('Department')" class="help is-danger">{{ errors.first('Department') }}</span>
                </div>
            </div>
            <div class="control">
                <button @click="sendForm()" class="button is-primary">Create User</button>
                &nbsp;
                ...
                this.$validator.validateAll().then((result) =>
                {
                    if (result)
                    {
                        ...
                        this.$store.dispatch('CreateUser', user).then(()=> {
                            ...
                            console.log(error);
                            this.$swal('Oops...', 'Failed to create user', 'error');
                        });
                    }
                });
            }
